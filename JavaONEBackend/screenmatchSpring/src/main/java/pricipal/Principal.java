...
public class Principal {
        ...
        if (serieBuscada.isPresent()){
            ...
            topEpisodios.forEach(episodio ->
                    System.out.printf("Serie %s - Temporada: %s - Episodio: %s - Titulo: %s - Evaluacion: %s\n",
                            episodio.getSerie().getTitle(),episodio.getTemporada(), episodio.getNumeroEpisodio(),episodio.getTitulo(), episodio.getEvalacion()));
        }
    }
    ...
    private void buscarEpisodioPorNombre() {
        ...
        episodiosEcontrados.forEach(episodio ->
                System.out.printf("Serie %s Temporada: %s Episodio: %s Evaluacion: %s\n",
                        episodio.getSerie().getTitle(),episodio.getTemporada(), episodio.getTitulo(), episodio.getEvalacion()));
    }
    ...
    public void filtrarSeriesPorTemporadaYEvaluacion(){
        ...
        filtroSeries.forEach(serie -> System.out.println("Serie:"+  serie.getTitle()+ "Eval:"+  serie.getEvaluacion()));
    }
    ...
    private void buscarTop5Series() {
        ...
        top5Series.forEach(serie -> System.out.println("Serie:"+  serie.getTitle()+ "Eval:"+  serie.getEvaluacion()));
    ...
    private void buscarEpisodiosPorSerie(){
        ...
        if (serie.isPresent()){
            ...
            for (int i = 1; i <= serieBuscada.getTotalTemporadas(); i++) {
                var json = consumoAPI.obtenerDatos(URL_BASE + serieBuscada.getTitle().replace(" ", "+") + "&season=" + i + API_KEY);
